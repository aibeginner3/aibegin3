...
def word_freq(mlines, word, outfile):
    ...
    #print(plot_hist)
    fout = open(outfile, 'w')
    print(hist, file=fout)
    fout.close()

    return plot_hist

def word_freq_detail(mlines, outfile):
    hist = dict() # キーに名詞、値に頻度（数字）が入るのを想定
    morphs = []
    tr_hist = dict()
    plot_hist = dict()
...
# カウントした単語をグラフで表示させる
def plot(countedwords, word, pngname):
    counts = {}
    ...
    plt.title('{0} 頻出語ベスト{1} : 総単語数{2} : 単語の種類数{3}'.format(word,show,total,len(countedwords)), size=16)
